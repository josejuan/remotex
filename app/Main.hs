...
mouseDelta display window dx dy = X11.selectInput display window X11.keyReleaseMask
                               >> X11.warpPointer display 0 0 0 0 0 0 dx dy
                               >> X11.flush display

mouseButton display button mode = X11.xFakeButtonEvent display button (mode == "down") 0
                               >> X11.flush display

mouseScroll display d = do
  let button = if d < 0 then 4 else 5
  mapM_ (\_ -> X11.xFakeButtonEvent display button True 0 >> X11.xFakeButtonEvent display button False 0) [1..abs d]
  X11.flush display

keyAction display down code = X11.xFakeKeyEvent display code down 0 >> X11.flush display
